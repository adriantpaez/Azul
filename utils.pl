...
%
inside(L,I) :- I >= 0, length(L,EC), I < EC.

% getByIndex(L,I,E)
% Devuelve en E el elemento que está en la posición I de L
%
getByIndex([X|_],0,X) :- !.
getByIndex([X|R],I,E) :- inside([X|R],I), NewIndex is I-1, getByIndex(R,NewIndex,E).

% removeValue(L,X,R)
% Devuelve en R el resultado de eliminar laprimera ocurrencia de X en L
removeValue([X|R],X,R) :- !.
removeValue([Y|R],X,[Y|ListResult]) :- removeValue(R,X,ListResult).
...
countCountinous(W,I,_,I,0) :-  getByIndex(W,I,0), !.
countCountinous(W,I,_,I,1) :-  getByIndex(W,I,1), !.
countCountinous(W,I,_,_,0) :-  getByIndex(W,I,0), !.
countCountinous(W,I,J,T,P) :-  getByIndex(W,I,1), 
                                K is I + J, 
