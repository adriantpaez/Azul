...
insertTile([Color,ColorCount], Count, [Color,CountResult]) :- CountResult is ColorCount + Count.
...
% 
pushNColorVector([[Color,Count]|ColorVector],Color,N,[[Color,NewCount]|ColorVector]) :- NewCount is Count + N, !.
pushNColorVector([[OtherColor,Count]|ColorVector],Color,N,[[OtherColor,Count]|ColorVectorResult]) :- pushNColorVector(ColorVector,Color,N,ColorVectorResult).
...
% 
floorPoint([],0) :- !.
floorPoint([[false,_]|R],Points) :- floorPoint(R,Points), !.
floorPoint([[true,Value]|R],Points) :- floorPoint(R,RPoints), Points is RPoints + Value.
...
columnPoints(W,I,TU,P) :-  TD is TU+20, 
...
wallPoints(W,I,0) :-   getByIndex(W,I,0), !.
wallPoints(W,I,P) :-   TL is (I/5) * 5, 
                        rowPoints(W,I,TL,PR), 
                        TU is I mod 5, 
...
initializebag([[BlueTuple,YellowTuple,RedTuple,BlackTuple, WhiteTuple],[0,1,2,3,4]]) :-    colorTuple(azul,20,BlueTuple),
...
takeOneBag([BagVector,Mask],Index,[BagVectorResult,MaskResult],ColorResult) :-  getByIndex(BagVector,Index,ColorTuple),
                                                                                ...
                                                                                !.
takeOneBag([BagVector,Mask],Index,[BagVectorResult,Mask],ColorResult) :-    getByIndex(BagVector,Index,ColorTuple),
...
takeNBag([BagVector,Mask], Count, Factory,FactoryResult ,[BagVectorResult,MaskResult]) :-   length(Mask,R),
                                                                                            random(0,R,I),
                                                                                            getByIndex(Mask,I,J),
...
makeNFactories(N,Bag,[FactoryResult|RFactoryList],BagResult) :- factoryEmpty(Factory),
